...
function parseCliArgs(args) {
  // Supports flags appearing before or after the URL, e.g.:
  //   node src/cli/index.js --format json-pretty https://react.dev/
  //   node src/cli/index.js https://react.dev/ --format=json-pretty
  //
  // We treat the URL as the *last* positional argument that wasn't consumed
  // as a value for a known flag.
  const valueFlags = new Set(["--format", "--max-width", "--inspect"]);
  const flags = new Set();
  const positionals = [];
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a.startsWith("--")) {
      ...
      if (eqIdx !== -1) {
        const name = a.slice(0, eqIdx);
        flags.add(name);
        continue; // value is still discoverable via getFlagValue()
      }

      flags.add(a);

      // If this flag takes a value, consume the next token so it won't be
      // mistaken for the URL.
      if (valueFlags.has(a)) {
        const next = args[i + 1];
        if (next && !next.startsWith("-")) i++; // skip consumed value
      }
      continue;
    }

    // Any other token is positional (URL candidate, etc.)
    positionals.push(a);
  }

  return {
    flags,
    url: positionals.length ? positionals[positionals.length - 1] : null,
  };
}
